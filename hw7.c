...
//    BSP_LCD_DisplayStringAtLine( 1, (uint8_t *)" Welcome to Nucleo! " );
    ...
    memset (btInputBuffer , '\0', cBUF_SIZE);            // set input buffer to NULL (entire array is NULL chars)
    ...
    while(1) // main loop
    {
        ...
        if( oRecvStatus == HAL_OK )
        {
            ...
            if (uiSerRecv != 0xD)
            {
                ...
                btInputBuffer[iItemsInBuffer] = chArr[0];
                iItemsInBuffer++;
        ...
        */
        ...
        {
            memset (btInputBuffer , '\0', cBUF_SIZE);           // clear printing buffer
            iItemsInBuffer = 0;
            ...
            if (iResult) // if not NULL
            {
                int iXs, iYs, iRad;
                int iResult;
                iResult = sscanf(chArrTmpData, "DRAW:CIRCLE %d,%d,%d", &iXs, &iYs, &iRad);  // number of items read from string, else -1
                // iXs, iYs, iRad change to 64, 80, 30 after this
                if (iResult > 2) // we read 3 > 2 items from chArrTmpData
                {
                    BSP_LCD_DrawCircle(iXs, iYs, iRad); // actual circle-drawing command
                }
                else
                {
